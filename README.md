...
- **Production**: Check deployment settings on Render
...
#### Get User Activities
```http
GET /users/{user_id}/activities?limit=50
```

#### Update Activity
```http
PUT /users/{user_id}/activities/{activity_id}
Content-Type: application/json

{
  "quantity": 15.0,
  "monetary_amount": 30.00
}
```
...
#### Physical Emissions Summary
```http
GET /users/{user_id}/summary/physical
```

#### Economic Impact Analysis
```http
GET /users/{user_id}/summary/economic
```

#### Summary by Category
```http
GET /users/{user_id}/summary/by-category
```

#### Biggest Carbon Impactors
```http
GET /users/{user_id}/summary/biggest-impactors
```

#### Time-Based Summaries
```http
GET /users/{user_id}/summary/daily
GET /users/{user_id}/summary/weekly
...
```http
GET /emission-factors?category=Transportation&search=flight
```

**Full API Documentation**: Visit `http://localhost:8000/docs` (development) for interactive Swagger UI
